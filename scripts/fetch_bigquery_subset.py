...
def main() -> None:
    ...
    # Use credentials from environment variable if set, otherwise from config
    if "GOOGLE_APPLICATION_CREDENTIALS" not in os.environ:
        if settings.gcp.credentials_path:
            cred_path = Path(settings.gcp.credentials_path)
            if cred_path.exists():
                os.environ["GOOGLE_APPLICATION_CREDENTIALS"] = str(cred_path)
            else:
                # Try relative to project root
                root_cred_path = Path(__file__).parent.parent / cred_path
                if root_cred_path.exists():
                    os.environ["GOOGLE_APPLICATION_CREDENTIALS"] = str(root_cred_path)
                else:
                    # Try absolute path from key directory
                    key_cred_path = Path("/home/sujals2144/project/key/patentsphere.json")
                    if key_cred_path.exists():
                        os.environ["GOOGLE_APPLICATION_CREDENTIALS"] = str(key_cred_path)
