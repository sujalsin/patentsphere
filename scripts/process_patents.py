...
- Support multiple chunking strategies: legacy (word-based), hybrid (semantic + hierarchical)
...
Usage Examples:
    ...
    # Use hybrid chunking for better RAG retrieval
    python scripts/process_patents.py --chunking-strategy hybrid
...
CHUNKING_STRATEGIES = ["legacy", "hybrid"]
...
class Chunk:
    ...
    # New fields for hybrid chunking
    chunk_level: int = 0  # 0 = parent, 1 = child
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=getattr(chunking_cfg, "strategy", "legacy"),
        help=(
            "Chunking strategy: 'legacy' (word-based) or 'hybrid' (semantic + hierarchical). "
            "Hybrid provides better RAG retrieval with parent-child relationships."
        ),
    )
    parser.add_argument(
        "--semantic-chunk-size",
        type=int,
        default=400,
        help="Target words per semantic chunk (hybrid mode only).",
    )
    parser.add_argument(
        "--split-threshold",
        type=int,
        default=500,
        help="Word count threshold to trigger semantic splitting (hybrid mode only).",
    )
    parser.add_argument(
        "--generate-embeddings",
        action="store_true",
...
def generate_embeddings(
    ...
    # Build encode kwargs, excluding num_workers if model doesn't support it
    encode_kwargs = {
        "batch_size": batch_size,
    ...
    # Only add num_workers if > 0 (some models don't support it)
    if num_workers > 0:
        try:
            # Test if model supports num_workers by checking its signature
            import inspect
            sig = inspect.signature(model.encode)
            if 'num_workers' in sig.parameters:
                encode_kwargs["num_workers"] = num_workers
        except Exception:
            pass  # Skip num_workers if we can't verify support
...
def write_stats(
    ...
    if chunking_strategy == "hybrid":
        parent_chunks = sum(1 for c in chunks if c.chunk_level == 0)
        child_chunks = sum(1 for c in chunks if c.chunk_level == 1)
        stats["parent_chunks"] = parent_chunks
        stats["child_chunks"] = child_chunks
        
        # Section type distribution
        section_types: Dict[str, int] = {}
        for c in chunks:
            section_types[c.section_type] = section_types.get(c.section_type, 0) + 1
        stats["section_distribution"] = section_types
    
    with output_path.open("w", encoding="utf-8") as f:
        json.dump(stats, f, indent=2)


def chunk_patent_hybrid(
    record: Dict,
    ...
    """
    Chunk a patent using the hybrid (semantic + hierarchical) strategy.
    
    This strategy:
    1. Preserves document structure (title, abstract, claims as separate chunks)
    2. Splits large sections at sentence boundaries
    3. Maintains parent-child relationships for retrieval expansion
    """
    from scripts.chunking.hybrid import HybridChunker
    
    chunker = HybridChunker(
        target_chunk_size=target_chunk_size,
        split_threshold=split_threshold,
        max_chunks_per_patent=max_chunks,
        include_context=True,
    )
    
    hybrid_chunks = chunker.chunk_patent(record)
    
    # Convert HybridChunk to Chunk dataclass
    chunks: List[Chunk] = []
    for hc in hybrid_chunks:
        chunks.append(Chunk(
            chunk_id=hc.chunk_id,
            patent_id=hc.patent_id,
            chunk_type=hc.chunk_type,
            text=hc.text,
            order=hc.order,
            publication_date=hc.metadata.get("filing_date"),
            cpc_codes=hc.metadata.get("cpc_codes"),
            chunk_level=hc.chunk_level,
            parent_chunk_id=hc.parent_chunk_id,
            section_type=hc.section_type,
            weight=hc.weight,
            leading_context=hc.leading_context,
            trailing_context=hc.trailing_context,
        ))
...
def main() -> None:
    ...
    chunking_strategy = getattr(args, "chunking_strategy", "legacy")
    
    if chunking_strategy == "legacy":
    ...
    for record in iter_patents(input_path, args.max_patents):
        if chunking_strategy == "hybrid":
            # Use hybrid chunking
            patent_chunks = chunk_patent_hybrid(
                record,
                target_chunk_size=getattr(args, "semantic_chunk_size", 400),
                split_threshold=getattr(args, "split_threshold", 500),
    ...
    if chunking_strategy == "hybrid":
        parent_count = sum(1 for c in chunks if c.chunk_level == 0)
        child_count = sum(1 for c in chunks if c.chunk_level == 1)
    ...
    write_stats(chunks, stats_path, patents_processed, chunking_strategy)
