...
{{
  ...
  "detailed_description": "Referring now to FIG. 1, a system architecture block diagram illustrates the principal hardware and software components of the preferred embodiment of the ingestion verification system 100. The system 100 comprises a mobile computing device 102 having an optical sensor 104, a processor 106, and non-transitory memory 108. Executing on the processor 106 from the non-transitory memory 108 are: a machine-learning inference module 110, a medication schedule module 112, and a local encrypted data store 114. An optional FHIR reporting interface 116 is communicatively coupled to the local encrypted data store 114 and operates subject to a user consent gate 118.\\n\\nReferring now to FIG. 2, a process flow diagram depicts the steps of the ingestion verification method 200 performed by the system 100. At step 202, the medication schedule module 112 retrieves the user-specific dosing schedule from the local encrypted data store 114 and determines that a scheduled pharmaceutical administration event is imminent within a configurable pre-administration time window, nominally set to five minutes before the scheduled administration time. At step 204, the medication schedule module 112 activates the optical sensor 104 and initiates a continuous video capture session, acquiring successive image frames depicting the oropharyngeal region of the user at a nominal frame rate of thirty frames per second.\\n\\nAt step 206, each acquired image frame is transmitted to the machine-learning inference module 110. In the preferred embodiment, the machine-learning inference module 110 comprises a two-stage convolutional neural network architecture. The first stage 110a processes each frame independently to detect and localize anatomical landmarks of the oropharyngeal region, including the uvula, posterior pharyngeal wall, and tonsillar pillars, generating a bounding box and landmark confidence map for each frame. The second stage 110b receives a temporal sequence of N consecutive first-stage outputs and applies a temporal classification model to classify the sequence as depicting a swallowing-motion event, generating a binary classification output and an associated confidence score. When the confidence score exceeds a configurable detection threshold, nominally set at 0.85, the inference module 110 generates an ingestion verification record comprising: the binary classification result; the confidence score; a UTC timestamp of the detected swallowing event; and a session identifier linking the record to the current medication administration event.\\n\\nIn an alternative embodiment, the machine-learning inference module 110 is implemented as a transformer-based temporal attention model that processes the complete video sequence as a single input, applying multi-head self-attention across the temporal dimension to identify swallowing-motion signatures without the two-stage landmark detection preprocessing step. This alternative embodiment provides improved robustness to variable user positioning and oropharyngeal region visibility conditions at the cost of increased computational overhead on the device processor 106. In a further alternative embodiment, the medication schedule module 112 is omitted, and the optical sensor 104 is activated manually by the user via a dedicated user interface control, enabling use cases where the user prefers to self-initiate the verification session rather than rely on automated schedule-driven activation.\\n\\nThe ingestion verification record generated at step 206 is encrypted by the local encrypted data store 114 using AES-256 symmetric encryption with a device-specific key derived from the device secure enclave prior to persistence at step 208, ensuring that the record cannot be accessed by any process lacking the device-specific decryption key. Each record is additionally cryptographically signed using an asymmetric key pair managed by the device secure enclave to provide tamper-evident audit-chain integrity suitable for regulatory compliance contexts. The encrypted, signed records accumulate in the local encrypted data store 114 and are accessible to the optional FHIR reporting interface 116 exclusively upon receipt of a digitally-signed user consent token, which initiates a selective export of the records to a clinician-authorized remote endpoint in FHIR Release 4 compliant MedicationStatement resource format.\\n\\nThe system 100 and method 200 are operable on any mobile computing device 102 having a front-facing optical sensor 104 with a minimum resolution of 720p and a processor 106 capable of executing the machine-learning inference module 110 at real-time frame rates, which encompasses the majority of consumer smartphones, tablets, and wearable computing devices manufactured after 2018. No dedicated hardware peripherals, external network connectivity, or cloud computing resources are required during the ingestion verification process itself, enabling deployment in low-connectivity clinical environments including rural outpatient clinics and assisted living facilities."
}}
