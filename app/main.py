...
def create_app() -> FastAPI:
    ...
    @app.get("/analyze", dependencies=[Depends(require_local_profile)])
    async def analyze_endpoint(query: str):
        """
        Enhanced analysis endpoint that returns the structured API response format
        with indexed, clickable citations.
        
        Returns:
            APIResponse with sources_header, answer_section, and detailed sources
        """
        results = await orchestrator.run_all(query)
        
        # Extract the enhanced API response from the final result
        final_result = results.get("final")
        if final_result and final_result.success:
            final_data = final_result.data
            
            # Return the enhanced API response if available
            if "api_response" in final_data:
                return final_data["api_response"]
            
            # Fallback to legacy format
            return {
                "response_id": f"resp_{id(results) % 100000:05d}",
                "query": query,
                "sources_header": "",
                "answer_section": {
                    "technical_summary": final_data.get("executive_summary", ""),
                    "legal_summary": None,
                    "novelty_assessment": None,
                },
                "sources": [],
                "risk_score": final_data.get("synthesis", {}).get("risk_score", 50),
                "quality_score": final_data.get("quality_score", 0.0),
                "metadata": final_data.get("metadata", {}),
            }
        
        # No final result - return error response
        return {
            "response_id": "error",
            "query": query,
            "error": "Analysis failed to complete",
            "results": {k: v.data for k, v in results.items() if v.success},
        }
